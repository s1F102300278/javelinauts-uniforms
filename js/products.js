const products = {

  /* ========================================
     青ユニ 半袖
  ======================================== */

  "blue-short": {

    name: "青ユニ 半袖",

    required: true,

    price: "¥4500",

    images: [
      "images/uniforms/blue1.png",
      "images/uniforms/blue2.png",
      "images/uniforms/blue3.png",
      "images/uniforms/blue4.png",
      "images/uniforms/blue5.png",
      "images/uniforms/blue6.png",
      "images/uniforms/blue7.png",
      "images/uniforms/blue8.png",
      "images/uniforms/blue9.png"
    ],

    design: `競技のために生まれたウェアでありながら、その佇まいは極めて静かである。

深いネイビー、純白、そしてブラック。ベースカラーを潔く抑えながら、袖を走るゴールドとホワイト、ネイビーのラインが、視線を一筋に導く。過度な装飾を排し、必要なものだけを残したデザインは、スポーツウェアでありながらどこかテーラードウェアを思わせる緊張感を漂わせる。

フロントに配された「JAVELINAUTS」の文字とエンブレムは、単なるチームロゴではない。この一着が所属するコミュニティと、その歴史を静かに刻むシグネチャーとして機能する。

そしてバックに大胆に配置されたナンバリング。フロントの繊細なグラフィックとの対比によって、背面には明確な存在感が生まれている。特にネイビー×ホワイトの組み合わせは、クラシックなスポーツウェアの文脈を感じさせながら、現代的なミニマリズムへと昇華されている。

袖口へ向かって流れるラインは、このウェアにおける象徴的なディテールだ。ゴールドが持つ品格、ホワイトの清潔感、そしてネイビーの落ち着き。その三色が一本の線として共存することで、静かなデザインの中に確かな躍動感を与えている。

競技のための機能性と、チームのアイデンティティ。

その二つを単なる「ユニフォーム」としてではなく、一着のプロダクトとして成立させた完成度が魅力だ。

フィールドに立つための服でありながら、フィールドを離れてもその意味を失わない。
勝利だけではなく、そこに至るまでの時間、仲間との記憶、そして背負う番号までもが、この一着には宿っている。

JAVELINAUTSというチームを着る。

そんな言葉がふさわしい、競技ウェアの枠を超えた一着である。`,

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "shirt",

  measurements: [
    {
      key: "A",
      label: "胸囲（周囲）"
    },
    {
      key: "B",
      label: "着丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B"
  ],

  rows: [
    ["XS", "91.5", "68.5"],
    ["S", "96.5", "71.0"],
    ["M", "101.5", "74.0"],
    ["L", "106.5", "76.0"],
    ["XL", "111.5", "79.0"],
    ["2XL", "116.5", "81.0"]
  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "身長:176cm、体重:60kg、性格:穏やか\n 購入サイズはLです。"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "「「レンジここに入る文章」」"
  },
  {
    name: "南谷爽太郎[#35]",
    rating: 4,
    comment: "俺が着るにふさわしいユニ"
  },
  {
    name: "成井佳貴[#6]",
    rating: 3,
    comment: "笑える"
  },
  {
    name: "白田翼[#74]",
    rating: 5,
    comment: "2026年学生予選の車が楽しかったです"
  },
  {
    name: "川尻堀大輔[#18/30min]",
    rating: 1,
    comment: "寝巻としては悪くないけど、睡眠改善は感じられなかった。"
  }

]

  },


  /* ========================================
     白ユニ 半袖
  ======================================== */

  "white-short": {

    name: "白ユニ 半袖",

    required: true,

    price: "¥4500",

    images: [
      "images/uniforms/white1.png",
      "images/uniforms/white2.png",
      "images/uniforms/white3.png",
      "images/uniforms/white4.png",
      "images/uniforms/white5.png",
      "images/uniforms/white6.png",
      "images/uniforms/white7.png",
      "images/uniforms/white8.png"
    ],

    design:
      "東洋大学ジャベリノーツの白ユニフォームになっています。大会に出場を考えている場合は購入必須となっています。\n\nかっこよさは青ユニに劣る上に汚れも目立つ。ただそれがいいのだ、白ユニがあることで青ユニのありがたみを知れる。\nつまりユニ界の縁の下の力持ちということだ。これはチームプレイであるアルティメットにおいてとてもおおきな意味をもつのである",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "shirt",

  measurements: [
    {
      key: "A",
      label: "胸囲（周囲）"
    },
    {
      key: "B",
      label: "着丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B"
  ],

  rows: [
    ["XS", "91.5", "68.5"],
    ["S", "96.5", "71.0"],
    ["M", "101.5", "74.0"],
    ["L", "106.5", "76.0"],
    ["XL", "111.5", "79.0"],
    ["2XL", "116.5", "81.0"]
  ],

  unit: "cm"

},

    reviews: [
       {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "身長:176cm、体重:60kg、性格:穏やか\n 購入サイズはLです。"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "汚せば汚すほどかっこいいです"
  }
    ]

  },


  /* ========================================
     パンツ
  ======================================== */

  "pants": {

    name: "パンツ",

    required: true,

    price: "¥4,000〜¥4,800",

    images: [
      "images/uniforms/pants1.png",
      "images/uniforms/pants2.png",
      "images/uniforms/pants3.png",
      "images/uniforms/pants4.png",
      "images/uniforms/pants5.png",
      "images/uniforms/pants6.png"
    ],

    design:
      "2026年から採用されたブラックのパンツです。大会に出場を考えている場合は購入必須になっています。\n\nブラックという色がどのユニフォームにも合うのがこのパンツのいいところだ。家ではもちろんパジャマとしても大活躍さ。\nただ部屋着としてはポッケがないところ少し残念だ。このブラックのパンツに青ユニを合わせて大会の会場を歩くと、他大学が道を開けてくれます。そう、このパンツは東洋大学ジャベリノーツの強さの象徴なんです。",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "pants",

  measurements: [
    {
      key: "A",
      label: "ウエスト（周囲）"
    },
    {
      key: "B",
      label: "パンツ丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B"
  ],

  rows: [
    ["XS", "70", "48"],
    ["S", "72", "51"],
    ["M", "75", "53"],
    ["L", "77", "56"],
    ["XL", "80", "58"],
    ["2XL", "82", "61"]
  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "身長:176cm、体重:60kg、性格:穏やか\n 購入サイズはLです。"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 3,
    comment: "俺がこの黒パンツを考案しじゃべに導入しました。"
  },
  {
    name: "川中日翔[#90]",
    rating: 2,
    comment: "前の白のパンツの方がよかった。だれだよ考案したやつ"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 3,
    comment: "やんのかてめえ"
  }
    ]

  },


  /* ========================================
     青ユニ 長袖
  ======================================== */

  "blue-long": {

    name: "青ユニ 長袖",

    required: false,

    price: "¥5500",

    images: [
      "images/uniforms/blue_long1.png",
      "images/uniforms/blue_long2.png",
      "images/uniforms/blue_long3.png",
      "images/uniforms/blue_long4.png",
      "images/uniforms/blue_long5.png",
      "images/uniforms/blue_long6.png",
      "images/uniforms/blue_long7.png",
      "images/uniforms/blue_long8.png"
    ],

    design:
      "半袖の青ユニの長袖バージョンです。このユニでも公式戦には出場できます。\n\n袖が長い分、半袖ユニよりも暖かいです。そのため、多少風邪が引きにくくなっており、病弱体質にも優しい設計です。\n長袖の方が玄人感が出て、買わない理由がない。",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "long-shirt",

  measurements: [
    {
      key: "A",
      label: "胸囲（周囲）"
    },
    {
      key: "B",
      label: "着丈"
    },
    {
      key: "C",
      label: "袖丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B",
    "C"
  ],

  rows: [
    ["XS", "91.5", "68.5", "58.5"],
    ["S", "96.5", "71.0", "61.0"],
    ["M", "101.5", "74.0", "62.0"],
    ["L", "106.5", "76.0", "64.5"],
    ["XL", "111.5", "79.0", "66.0"],
    ["2XL", "116.5", "81.0", "67.0"]
  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "もっと早く作っていれば絶対欲しかった。"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 3,
    comment: "かっこいい"
  }
    ]

  },


  /* ========================================
     白ユニ 長袖
  ======================================== */

  "white-long": {

    name: "白ユニ 長袖",

    required: false,

    price: "¥5500",

    images: [
      "images/uniforms/white_long1.png",
      "images/uniforms/white_long2.png",
      "images/uniforms/white_long3.png",
      "images/uniforms/white_long4.png",
      "images/uniforms/white_long5.png",
      "images/uniforms/white_long6.png",
      "images/uniforms/white_long7.png",
      "images/uniforms/white_long8.png"
    ],

    design:
      "半袖の白ユニの長袖バージョンです。このユニでも公式戦には出場できます。\n\n袖が長い分、体の白色の占める面積が広くなります。そう、白は膨張色です。どんなに筋トレをしても体がでかくならないそこのあなた、もう大丈夫です。このユニを着れば相手から見たあなたは1.8倍くらいにはでかく見えているはずです。自信をもってマッチアップできます。",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "long-shirt",

  measurements: [
    {
      key: "A",
      label: "胸囲（周囲）"
    },
    {
      key: "B",
      label: "着丈"
    },
    {
      key: "C",
      label: "袖丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B",
    "C"
  ],

  rows: [
    ["XS", "91.5", "68.5", "58.5"],
    ["S", "96.5", "71.0", "61.0"],
    ["M", "101.5", "74.0", "62.0"],
    ["L", "106.5", "76.0", "64.5"],
    ["XL", "111.5", "79.0", "66.0"],
    ["2XL", "116.5", "81.0", "67.0"]
  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "かっこいい"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "かっこいい"
  }
    ]

  },


  /* ========================================
     黒ユニ 長袖
  ======================================== */

  "black-long": {

    name: "黒ユニ 長袖",

    required: false,

    price: "¥5500",

    images: [
      "images/uniforms/black_long1.png",
      "images/uniforms/black_long2.png",
      "images/uniforms/black_long3.png",
      "images/uniforms/black_long4.png",
      "images/uniforms/black_long5.png",
      "images/uniforms/black_long6.png",
      "images/uniforms/black_long7.png",
      "images/uniforms/black_long8.png"
    ],

    design:
      "こちらは他のユニフォームと異なり、番号やロゴの主張が強すぎない着やすいデザインになっています。残念ながら公式試合では着用できないですが、ベースカラーにブラックを採用しているため、絶対にかっこよくなってしまう一着になっています。\n\n特徴としては背番号の代わりに胸部分にじゃべの2026年新人大会本選出場記念ロゴと選手番号を配置しており、他のユニにはない特別感が味わえます。\nこの記念ロゴがフェラーリのシンボルのようで、車好きにはたまらないだろう。",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "long-shirt",

  measurements: [
    {
      key: "A",
      label: "胸囲（周囲）"
    },
    {
      key: "B",
      label: "着丈"
    },
    {
      key: "C",
      label: "袖丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B",
    "C"
  ],

  rows: [
    ["XS", "91.5", "68.5", "58.5"],
    ["S", "96.5", "71.0", "61.0"],
    ["M", "101.5", "74.0", "62.0"],
    ["L", "106.5", "76.0", "64.5"],
    ["XL", "111.5", "79.0", "66.0"],
    ["2XL", "116.5", "81.0", "67.0"]
  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "かっこいい"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "俺が考えたんだからかっこいいに決まってる"
  }
    ]

  },


  /* ========================================
     スリーブレス
  ======================================== */

  "sleeveless": {

    name: "スリーブレス",

    required: false,

    price: "¥4100",

    images: [
      "images/uniforms/sleeveless1.png",
      "images/uniforms/sleeveless2.png",
      "images/uniforms/sleeveless3.png",
      "images/uniforms/sleeveless4.png",
      "images/uniforms/sleeveless5.png",
      "images/uniforms/sleeveless6.png",
      "images/uniforms/sleeveless7.png",
      "images/uniforms/sleeveless8.png"
    ],

    design:
      "こちらはかっこいいピンクアクセントが際立つスリーブレスのユニフォームになっています。公式戦では着用できません。\n\nなんといっても肩のピンクの三本線。まるでアディ〇スのようですね。黒ベースで一見シンプルに見えますが、よく見ると下から上にかけて幾何学模様が施されています。主張が強すぎないことで胸部のじゃべのロゴにスポットライトがあたります。通称：アディダスユニ、ぜひご検討を。",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "sleeveless",

  measurements: [
    {
      key: "A",
      label: "身幅"
    },
    {
      key: "B",
      label: "着丈"
    }
  ],

  groups: [

    {
      name: "MEN'S",

      headers: [
        "サイズ",
        "A",
        "B"
      ],

      rows: [
        ["XS", "48.3", "67.3"],
        ["S", "50.8", "69.9"],
        ["M", "53.3", "72.4"],
        ["L", "55.9", "74.9"],
        ["XL", "58.4", "77.5"],
        ["2XL", "61.0", "80.0"],
        ["3XL", "63.5", "82.6"],
        ["4XL", "66.0", "85.1"]
      ]
    },

    {
      name: "WOMEN'S",

      headers: [
        "サイズ",
        "A",
        "B"
      ],

      rows: [
        ["XS", "43.2", "62.2"],
        ["S", "45.7", "64.8"],
        ["M", "48.3", "67.3"],
        ["L", "50.8", "69.9"],
        ["XL", "53.3", "72.4"],
        ["2XL", "55.9", "74.9"]
      ]
    }

  ],

  unit: "cm",

  note:
    "メーカー表記（inch）をcmに換算し、小数第1位に丸めています。"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "かっこいい"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "かっこいい"
  }
    ]

  },


  /* ========================================
     タンクトップ
  ======================================== */

  "tanktop": {

    name: "タンクトップ",

    required: false,

    price: "¥4100",

    images: [
      "images/uniforms/tanktop1.png",
      "images/uniforms/tanktop2.png",
      "images/uniforms/tanktop3.png",
      "images/uniforms/tanktop4.png",
      "images/uniforms/tanktop5.png",
      "images/uniforms/tanktop6.png",
      "images/uniforms/tanktop7.png",
      "images/uniforms/tanktop8.png"
    ],

    design:
      "こちらはタンクトップのユニです。公式戦では着用できません。ベースカラーにブラックを採用しており、とてもスタイリッシュな一着になっています。\n\nスリーブレスのアディダスユニよりも肩の布面積が少なく、筋肉をアピールしたい方にはおすすめのユニフォームとなっています。\n特徴としては、背番号の代わりに胸元にじぇべ特別ロゴと選手番号を配置しており、主張が強すぎずかっこいい点です。\n筋肉バカは買わない理由がない。",

    material:
  "ポリエステル100％\n\n伸縮性のある素材で、乾きも早い。ポリエステル100％によるシワの付きにくさはぴかいちだ。\n\n練習や大会で着る以外では、パジャマや部屋着として一軍をキープしている。他のパジャマには申し訳ないが、このパジャマ...おっと、このユニフォームを着れば睡眠の質が段違いに最高級になる。堀大輔も30分睡眠では済まないだろう。",

    size: {

  type: "tanktop",

  measurements: [
    {
      key: "A",
      label: "胸囲（周囲）"
    },
    {
      key: "B",
      label: "着丈"
    }
  ],

  headers: [
    "サイズ",
    "A",
    "B"
  ],

  rows: [
    ["XS", "94", "69"],
    ["S", "97", "71"],
    ["M", "104", "74"],
    ["L", "107", "76"],
    ["XL", "109", "79"],
    ["2XL", "112", "81"]
  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "春から夏にかけて部屋着の不動のエースです"
  },
  {
    name: "森雄作[#5]",
    rating: 5,
    comment: "買わない理由がない。"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "かっこいい"
  }
    ]

  },


  /* ========================================
     ジャベパ
  ======================================== */

  "javepa": {

    name: "ジャベパ",

    required: false,

    price: "上下セット　¥7,500\n上のみ　　　¥3,900\n下のみ　　　¥3,600",

    images: [
      "images/uniforms/javepa1.png",
      "images/uniforms/javepa2.png",
      "images/uniforms/javepa3.png",
      "images/uniforms/javepa4.png",
      "images/uniforms/javepa5.png",
      "images/uniforms/javepa6.png",
      "images/uniforms/javepa7.png",
      "images/uniforms/javepa8.png",
      "images/uniforms/javepa9.png",
      "images/uniforms/javepa10.png"
    ],

    design:
      "いつデザインされたか分かりませんが、代々受け継がれたデザインのパーカーとスウェットパンツです。\n上は東洋大学を表すネイビーを採用していて、胸元には使用許可が出ているのかどうかも分からない東洋大学の公式シンボルがデザインされています。\n下は灰色のシンプルかつ履きやすいデザインを採用しています。\n\n個人的に夏以外は日常でもめちゃくちゃ愛用している。汚れてもそこまで精神的ダメージが大きくないのに、別にダサすぎないため、遠征時やバカ飲み会などでは他のユニフォームよりも活躍を見せてくれる。",

    material:
  "綿60％、ポリエステル40％\n\n履きやすい素材の上、じゃべの一員としての誇りが高まる。特別な一品だ。",

    size: {

  type: "javepa",

  sections: [

    {
      name: "TOPS",
      label: "ジャベパ 上",
      diagram: "hoodie",

      measurements: [
        {
          key: "A",
          label: "着丈"
        },
        {
          key: "B",
          label: "身幅"
        },
        {
          key: "C",
          label: "肩幅"
        },
        {
          key: "D",
          label: "袖丈"
        }
      ],

      headers: [
        "サイズ",
        "A",
        "B",
        "C",
        "D"
      ],

      rows: [
        ["S", "64", "54", "46", "61"],
        ["M", "67", "57", "49", "62"],
        ["L", "70", "60", "52", "63"],
        ["XL", "73", "63", "55", "64"],
        ["2XL", "76", "66", "58", "65"],
        ["3XL", "79", "69", "61", "66"],
        ["4XL", "82", "72", "64", "67"],
        ["5XL", "85", "75", "67", "68"],
        ["JS/110", "49", "38", "33", "41"],
        ["JM/130", "56", "45", "40", "49"],
        ["JL/150", "60", "49", "43", "56"]
      ]
    },

    {
      name: "BOTTOMS",
      label: "ジャベパ 下",
      diagram: "sweatpants",

      measurements: [
        {
          key: "A",
          label: "総丈"
        },
        {
          key: "B",
          label: "ウエスト"
        },
        {
          key: "C",
          label: "股下"
        }
      ],

      headers: [
        "サイズ",
        "A",
        "B",
        "C"
      ],

      rows: [
        ["XS", "98", "64〜72", "72"],
        ["S", "100", "70〜78", "75"],
        ["M", "104", "74〜82", "77"],
        ["L", "108", "77〜85", "78"],
        ["XL", "112", "80〜88", "80"],
        ["XXL", "114", "83〜91", "82"],
        ["XXXL", "116", "88〜100", "82"]
      ]
    }

  ],

  unit: "cm"

},

    reviews: [
      {
    name: "小松優之介[#25]",
    rating: 5,
    comment: "購入サイズ：L\n日常、特に飲み会で最大限の力を発揮してくれると感じました。とても気に入ってます。"
  },
  {
    name: "下山田蓮治[#12]",
    rating: 5,
    comment: "購入サイズ：LL\nかっこいい"
  },
  {
    name: "深瀬光[#23]",
    rating: 5,
    comment: "購入サイズ：L\nかっこいい"
  }
    ]

  }

};